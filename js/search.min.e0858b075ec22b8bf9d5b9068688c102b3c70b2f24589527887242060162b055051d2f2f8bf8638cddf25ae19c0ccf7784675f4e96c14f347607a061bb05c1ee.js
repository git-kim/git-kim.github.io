...
return false;}
let httpRequest=new XMLHttpRequest();httpRequest.onreadystatechange=function(){if(httpRequest.readyState===4&&httpRequest.status===200){const results=JSON.parse(httpRequest.responseText).filter(search);const numResults=results.length;if(numResults===0){header.innerText="No results found for "+query+".";return;}
...
return;}};httpRequest.open("GET","/index.json");httpRequest.send();})();